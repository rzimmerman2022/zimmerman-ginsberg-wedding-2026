# Vendor Requirements & Comparison Guide

This document outlines what to look for when researching and comparing wedding vendors.

---

## Venue

### Must-Have Requirements
- [ ] Available for October 2026
- [ ] Capacity for ____ guests (determine final count)
- [ ] Indoor/outdoor options
- [ ] Backup plan for weather
- [ ] Adequate parking or valet service
- [ ] Accessible facilities (ADA compliant)
- [ ] Getting-ready spaces for wedding party

### Questions to Ask
- What's included in the venue fee?
- Is catering in-house or can we bring our own?
- What time can we access the venue for setup?
- When must the event end?
- What's your cancellation/postponement policy?
- Do you provide tables, chairs, linens?
- Is there an on-site coordinator?
- What are the bar/alcohol policies?
- Any noise restrictions or curfews?
- What's not allowed at the venue?

### Evaluation Criteria
- Overall atmosphere and aesthetics
- Location accessibility for guests
- Value for money
- Flexibility with vendors
- Staff professionalism
- Reviews and references

---

## Photography

### Must-Have Requirements
- [ ] Available October 2026
- [ ] Experienced with Arizona weddings
- [ ] Portfolio matches desired style
- [ ] Engagement session included (optional)
- [ ] Full rights to digital images
- [ ] Minimum ____ hours of coverage
- [ ] Second shooter included

### Questions to Ask
- How many weddings have you photographed?
- Can we see a full wedding gallery, not just highlights?
- What's included in your packages?
- How long before we receive photos?
- Do you provide raw/unedited files?
- What's your backup equipment plan?
- Do you have liability insurance?
- How many photos can we expect?
- What's your editing style?
- Do you do engagement photos?

### Evaluation Criteria
- Photography style (traditional, photojournalistic, artistic)
- Personality fit (you'll spend all day with them)
- Technical quality and consistency
- Turnaround time
- Value for money
- Client testimonials

---

## Catering

### Must-Have Requirements
- [ ] Available October 2026
- [ ] Experience with venue (if applicable)
- [ ] Accommodates dietary restrictions
- [ ] Includes service staff
- [ ] Professional and licensed
- [ ] Tastings available
- [ ] Bar service (if needed)

### Questions to Ask
- Can we schedule a tasting?
- What's your price per person?
- What service styles do you offer (buffet, plated, family-style)?
- Are linens, china, and glassware included?
- How many staff will you provide?
- What's your ratio of servers to guests?
- Do you handle bar service?
- Can you accommodate dietary restrictions?
- When do you need final headcount?
- What's your payment schedule?

### Evaluation Criteria
- Food quality and presentation
- Menu flexibility
- Service quality and professionalism
- Value for money
- Reviews from recent clients
- Responsiveness to questions

---

## Florist

### Must-Have Requirements
- [ ] Available October 2026
- [ ] Experienced with Arizona climate
- [ ] Portfolio matches vision
- [ ] Delivers and sets up
- [ ] Works within budget
- [ ] Sources quality flowers

### Questions to Ask
- Can you work with my color palette?
- What flowers are in season in October?
- Do you provide rentals (vases, etc.)?
- When will flowers be delivered?
- Will you set up and break down?
- Do you have photos of past work at my venue?
- What's your refund/cancellation policy?
- Can you preserve my bouquet?
- Do you offer payment plans?

### Evaluation Criteria
- Design aesthetic and creativity
- Understanding of your vision
- Quality of flowers in portfolio
- Attention to detail
- Responsiveness
- Value for budget

---

## Music/Entertainment

### Must-Have Requirements
- [ ] Available October 2026
- [ ] Experience with weddings
- [ ] Appropriate repertoire
- [ ] Professional equipment and backup
- [ ] Liability insurance
- [ ] Emcee capabilities (for DJ)

### Questions to Ask
**For DJ:**
- How many weddings have you DJed?
- Can we provide a must-play/do-not-play list?
- Do you take requests from guests?
- What equipment do you provide?
- Do you have backup equipment?
- Will you emcee the reception?
- Can we meet before the wedding to discuss music?
- What's your attire?

**For Band:**
- What size is your band?
- What's your typical setlist?
- How many breaks do you take?
- Do you provide music during breaks?
- Can you learn specific songs?
- Have you played at our venue before?

### Evaluation Criteria
- Energy and performance quality
- Music variety and versatility
- Personality and professionalism
- Equipment quality
- Crowd engagement ability
- Reviews and references

---

## Hair & Makeup

### Must-Have Requirements
- [ ] Available October 2026
- [ ] Trial session included
- [ ] Experience with bridal styling
- [ ] Travel to venue/hotel
- [ ] Portfolio of previous work
- [ ] Uses quality products

### Questions to Ask
- Are trials included in the price?
- How far in advance should we do trials?
- What time will you arrive on the wedding day?
- How long does each person take?
- Do you bring an assistant?
- What products do you use?
- Do you have experience with my hair type/skin tone?
- What's your contingency plan if you're sick?
- Do you offer touch-up services during the event?

### Evaluation Criteria
- Portfolio quality
- Ability to achieve your desired look
- Personality and professionalism
- Punctuality
- Product quality
- Client reviews

---

## Officiant

### Must-Have Requirements
- [ ] Available October 2026
- [ ] Licensed in Arizona
- [ ] Experienced with ceremonies
- [ ] Willing to customize ceremony
- [ ] Attends rehearsal

### Questions to Ask
- Are you legally ordained in Arizona?
- How many weddings have you officiated?
- What's your ceremony style?
- Can we customize our ceremony?
- Do you provide a ceremony script?
- Will you attend the rehearsal?
- What's your fee?
- What's your backup plan if you can't make it?
- What do you wear?

### Evaluation Criteria
- Ceremony philosophy alignment
- Personality fit
- Speaking style and presence
- Flexibility with customization
- Professionalism
- References from other couples

---

## Videographer

### Must-Have Requirements (if hiring)
- [ ] Available October 2026
- [ ] Experienced with weddings
- [ ] Portfolio matches desired style
- [ ] Professional equipment
- [ ] Minimum ____ hours coverage
- [ ] Edited highlight reel included

### Questions to Ask
- What packages do you offer?
- How many hours of coverage?
- Do you use multiple cameras?
- What's your editing style?
- When will we receive the final video?
- Do you capture audio (vows, toasts)?
- Do you work with our photographer?
- What's included (highlight reel, full ceremony, raw footage)?
- Do you have drone capabilities?

### Evaluation Criteria
- Video quality and editing style
- Audio quality
- Storytelling ability
- Personality and discretion
- Turnaround time
- Value for money

---

## Transportation

### Must-Have Requirements
- [ ] Available October 2026
- [ ] Appropriate vehicle type
- [ ] Licensed and insured
- [ ] Professional chauffeur
- [ ] Backup plan

### Questions to Ask
- What vehicles do you have available?
- What's included in your hourly rate?
- Is there a minimum rental time?
- Are drinks/decorations included?
- What's your cancellation policy?
- Do you provide red carpet service?
- How early will the driver arrive?
- What's your fuel/gratuity policy?

### Evaluation Criteria
- Vehicle condition and cleanliness
- Driver professionalism
- Company reputation
- Insurance coverage
- Pricing transparency
- Flexibility with schedule

---

## Wedding Planner/Coordinator (Optional)

### Must-Have Requirements
- [ ] Available October 2026
- [ ] Experience with Arizona weddings
- [ ] Clear service packages
- [ ] Vendor relationships
- [ ] Organizational skills
- [ ] Emergency preparedness

### Questions to Ask
- What services are included?
- How many weddings do you plan per year?
- Will you be there on the wedding day?
- Do you have preferred vendors?
- How often will we meet?
- What's your planning process?
- Do you handle vendor payments?
- What's your emergency backup plan?

### Service Levels
- **Full Planning:** Start to finish (12-18 months)
- **Partial Planning:** Specific aspects only
- **Month-of Coordination:** Final 4-6 weeks + wedding day
- **Day-of Coordination:** Wedding day only

### Evaluation Criteria
- Organizational ability
- Personality match
- Communication style
- Vendor network quality
- Problem-solving skills
- Value for investment

---

## General Vendor Evaluation Checklist

For ALL vendors, consider:

### Before Booking
- [ ] Checked online reviews (Google, Yelp, WeddingWire, The Knot)
- [ ] Asked for and contacted references
- [ ] Verified licensing and insurance
- [ ] Reviewed complete contract
- [ ] Understood payment schedule
- [ ] Confirmed availability for your date
- [ ] Discussed backup plans

### Red Flags to Watch For
- ❌ No written contract
- ❌ Requests full payment upfront
- ❌ No liability insurance
- ❌ Poor communication/responsiveness
- ❌ No portfolio or references
- ❌ Unclear pricing or hidden fees
- ❌ Unwilling to meet in person/video
- ❌ No cancellation/refund policy
- ❌ Pressure tactics to book immediately

### Contract Must-Haves
- [ ] Specific date and time
- [ ] Detailed services description
- [ ] Complete pricing breakdown
- [ ] Payment schedule
- [ ] Cancellation policy
- [ ] Contingency plans
- [ ] Overtime rates (if applicable)
- [ ] Deliverables and timeline
- [ ] Liability and insurance info
- [ ] Signatures from both parties

---

## Arizona-Specific Considerations

### Venue
- Heat management for outdoor October events
- Backup indoor space for unexpected weather
- Adequate shade structures
- Hydration stations for guests
- Sunset timing (earlier in October)

### Photography
- Experience with Arizona lighting conditions
- Knowledge of local scenic locations
- Heat-appropriate timing for outdoor photos

### Catering
- Food safety in warm weather
- Appropriate service style for climate
- Bar service in heat

### Flowers
- Hardy varieties for Arizona climate
- Availability of October blooms
- Proper hydration and refrigeration

---

## Vendor Comparison Template

Use the template files (`venue-comparison-template.md`, `photographer-comparison-template.md`) to compare specific vendors side-by-side.

---

**Last Updated:** October 2025
